...
class CoinTrade extends Database {
    ...
    */
    ...
    public function insert($ip) {
        ...
        $query = "INSERT INTO $this->tableName (`user_id`,`coin_id`,`amount`,`usd_value`,`current_coin_price`,`createtime`,`createip`) VALUES (:user_id,:coin_id,:amount,:usd_value,:current_coin_price,:createtime,:createip)";
       ...
       $sql = 'SELECT '.$select.' FROM '.$this->tableName.' WHERE '.$where_str.'  ORDER BY created_at*1 DESC '.$limit.'  ';
		...
		$key = 0;
			for($i=0;$i<count($rows);$i++)
			{
				...
				$resp[$key]['date'] = !empty($rows[$i]['created_at'])?date('Y-m-d H:i:s',$rows[$i]['created_at']):'-';
				...
				$resp[$key+1]['date'] = !empty($rows[$i]['created_at'])?date('Y-m-d H:i:s',$rows[$i]['created_at']):'-';
...
print_r($data);exit;*/
        ...
        $query = "INSERT INTO $this->tableName (`user_id`,`coin_swap_from`,`coin_swap_to`,`swap_out_amout`,`swap_out_service_fee`,`swap_in_amout`,`swap_in_coin_price`,`swap_in_service_fee`,`created_at`,`created_ip`,`swap_out_coin_price`) VALUES (:user_id,:coin_swap_from,:coin_swap_to,:swap_out_amout,:swap_out_service_fee,:swap_in_amout,:swap_in_coin_price,:swap_in_service_fee,:created_at,:created_ip,:swap_out_coin_price)";
        ...
        $this->bind(':swap_out_service_fee', $data['swap_out_service_fee']);
        $this->bind(':swap_in_amout', $data['swap_in_amout']);
        $this->bind(':swap_in_coin_price', $data['swap_in_coin_price']);
        $this->bind(':swap_in_service_fee', $data['swap_in_service_fee']);
